...
$(function() {
  ...
  $("#valida_form").click(function() {
      ...
      $.fn.yiiactiveform.validate(form, function(messages) {
          if($.isEmptyObject(messages)) {
              ...
              $.ajax({ 
                  type: "POST", 
                  url: "<?php echo Yii::app()->createUrl('tesoreria/verifcheq'); ?>",
                  data: data,
                  beforeSend: function(){
                  ...
                  success: function(resp){
                      ...
                      if(resp == 0){
                        ...
                        $("#div_mensaje").html('<button type="button" class="close" aria-hidden="true" onclick="limp_div_msg();">×</button><h5><i class="icon fas fa-info-circle"></i>Info</h5>No se ha impreso ningún cheque con los criterios de busqueda utilizados.'); 
                      }

                      if(resp == 1){
                        $('html, body').animate({scrollTop:0}, 'fast');
                        $("#div_mensaje").addClass("alert alert-warning alert-dismissible");
                        $("#div_mensaje").html('<button type="button" class="close" aria-hidden="true" onclick="limp_div_msg();">×</button><h5><i class="icon fas fa-info-circle"></i>Info</h5>Este cheque ya fue impreso el n° de veces permitidas (3).');
                      }
  ...
  $("#print").click(function() {
    ...
    $.ajax({ 
        type: "POST", 
        url: "<?php echo Yii::app()->createUrl('tesoreria/regrimpcheq'); ?>",
        data: data,
        beforeSend: function(){
            $(".ajax-loader").fadeIn('fast'); 
        },
        success: function(resp){
            ...
            if(resp == 0){
              ...
              $('html, body').animate({scrollTop:0}, 'fast');
              $("#div_mensaje").addClass("alert alert-warning alert-dismissible");
              $("#div_mensaje").html('<button type="button" class="close" aria-hidden="true" onclick="limp_div_msg();">×</button><h5><i class="icon fas fa-info-circle"></i>Info</h5>No se registro la reimpresión del cheque '+cia+'-'+co+'-'+tipo+'-'+consecutivo+' en el sistema.');
            ...
            if(resp == 1){
              ...
              $('#valida_form').show();
               ...
               $('html, body').animate({scrollTop:0}, 'fast');
              $("#div_mensaje").addClass("alert alert-primary alert-dismissible");
              $("#div_mensaje").html('<button type="button" class="close" aria-hidden="true" onclick="limp_div_msg();">×</button><h5><i class="icon fas fa-info-circle"></i>Info</h5>Se registro la reimpresión del cheque '+cia+'-'+co+'-'+tipo+'-'+consecutivo+' en el sistema.');
