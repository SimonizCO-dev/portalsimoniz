...
$array_header['logo'] = Yii::app()->basePath.'/../images/logo_header_simoniz.jpg';
...
$pdf->Tabla();
ob_end_clean();
$nombre_archivo_gen = Yii::app()->basePath.'/../tmp/wip/WIP_'.$modelo_wip->WIP.'.pdf';
...
if($cadena_emails_adic != ""){
    ...
    if(file_exists($nombre_archivo_gen)){
        ...
        foreach ($array_emails as $llave => $email) {
            $resp = UtilidadesMail::enviowip($id ,$email, $nombre_archivo_gen);
            $num_notif = $num_notif + intval($resp);
        }

        unlink(Yii::app()->basePath.'/../tmp/wip/WIP_'.$modelo_wip->WIP.'.pdf');
    }
