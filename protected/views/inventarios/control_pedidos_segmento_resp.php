...
if($segmento != "" && $condicion_estados != "" && $condicion_tipos == ""){
  $opc = 1;
}
if($segmento != "" && $condicion_estados == "" && $condicion_tipos != ""){
  $opc = 2;
...
$query= "
    SET NOCOUNT ON
    EXEC P_PR_COM_CONT_PED_SEGMENTO
    @OPT = ".$opc.",
    @VAR1 = N'".$segmento."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(100,5,utf8_decode('CONTROL DE PEDIDOS POR SEGMENTO'),0,0,'L');
      $this->SetFont('Arial','',7);
      $this->Cell(95,5,utf8_decode($this->fecha_actual),0,0,'R');
      $this->Ln();
      $this->SetFont('Arial','',7);
      $this->Cell(195,5,utf8_decode('Criterio de búsqueda / Segmento: '.$this->segmento),0,0,'L');
      $this->Ln();
      $this->SetFont('Arial','',7);
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_segmento_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Control_pedidos_segmento_'.date('Y_m_d_H_i_s');
