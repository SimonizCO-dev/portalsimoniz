...
$query= "
    SET NOCOUNT ON
    EXEC P_PR_COM_CONT_PED_MARCA_LISTAS
    @OPT = ".$opc.",
    @VAR1 = N'".$marca."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(100,5,utf8_decode('CONTROL DE PEDIDOS POR MARCA (LISTAS)'),0,0,'L');
      $this->SetFont('Arial','',7);
      $this->Cell(95,5,utf8_decode($this->fecha_actual),0,0,'R');
      $this->Ln();
      $this->SetFont('Arial','',7);
      $this->Cell(195,5,utf8_decode('Criterio de búsqueda / Marca: '.$this->marca),0,0,'L');
      $this->Ln();
      $this->SetFont('Arial','',7);
      ...
      $this->Cell(11,2,utf8_decode('EXIST.'),0,0,'R');
      ...
      $this->Cell(11,2,utf8_decode('A LA FECHA'),0,0,'R');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_marca_listas_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'VALOR LISTA 001');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', 'VALOR LISTA 560');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', 'UND. COMPRA');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', 'PROM. VENTAS');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', 'STOCK MESES');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', 'BASE PEDIDOS');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', 'EXIST. A LA FECHA');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1', 'O.C PEND.');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M1', 'A.D PEDIR');
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N1', '# DÍAS CUBRIM.');
  ...
  foreach ($query1 as $reg1) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$Fila, $LISTA001);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$Fila, $LISTA560);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$Fila, $UND_COMPRA);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$Fila, $PROM_VENTAS);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$Fila, $STOCK_MESES);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$Fila, $BASE_PEDIDOS);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$Fila, $EXIST_FECHA);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$Fila, $O_C_PEND);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$Fila, $AD_PEDIR);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$Fila, $DIAS_CUB);

    $objPHPExcel->getActiveSheet(0)->getStyle('A'.$Fila.':D'.$Fila)->getAlignment()->setHorizontal($alignment_left);
    $objPHPExcel->getActiveSheet(0)->getStyle('E'.$Fila.':H'.$Fila)->getNumberFormat()->setFormatCode('0'); 
    $objPHPExcel->getActiveSheet(0)->getStyle('I'.$Fila)->getNumberFormat()->setFormatCode('#,##0.00');
    $objPHPExcel->getActiveSheet(0)->getStyle('J'.$Fila.':N'.$Fila)->getNumberFormat()->setFormatCode('0'); 
    $objPHPExcel->getActiveSheet(0)->getStyle('E'.$Fila.':N'.$Fila)->getAlignment()->setHorizontal($alignment_right);      
  ...
  $n = 'Control_pedidos_marca_listas_'.date('Y_m_d_H_i_s');
