...
$query= "
    SET NOCOUNT ON
    EXEC P_PR_COM_ACUM_VT_OR_EST_TIP
    @OPT = ".$opc.",
    @VAR1 = N'".$oracle."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,utf8_decode('CONTROL DE PEDIDOS ACUMULADO / ORACLE'),0,0,'L');
      $this->SetFont('Arial','',7);
      $this->Cell(140,5,utf8_decode($this->fecha_actual),0,0,'R');
      $this->Ln();
      $this->SetFont('Arial','',7);
      $this->Cell(340,5,utf8_decode('Criterio de búsqueda / Desc. oracle: '.$this->oracle),0,0,'L');
      $this->Ln();
      $this->SetFont('Arial','',7);
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Control_pedidos_acumulado_oracle_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Control_pedidos_acumulado_oracle_'.date('Y_m_d_H_i_s');
