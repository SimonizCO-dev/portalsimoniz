...
$linea_inicial = $model['linea_inicial'];
$linea_final = $model['linea_final'];
...
$query ="
  SET NOCOUNT ON
  EXEC P_PR_COM_RT_INV_LINEA_FECH
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,utf8_decode('RENTABILIDAD DE INVENTARIO POR LÍNEA'),0,0,'L');
      ...
      $this->Ln();
      $this->Cell(340,5,utf8_decode('Criterio de búsqueda: Línea de '.trim($this->linea_inicial).' a '.trim($this->linea_final)),0,0,'L');
      $this->Ln();
      $this->Ln();
      ...
      $this->Cell(10,2,utf8_decode('LÍNEA'),0,0,'L');
      ...
      foreach ($query1 as $reg1) {
        ...
        if($linea != $linea_act){
          ...
          if($linea != ""){
            ...
            $this->SetFont('Arial','B',6);
            $this->Cell(50,5,'TOTAL '.$linea,0,0,'R');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_linea_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'LÍNEA / ITEM');
  ...
  foreach ($query1 as $reg1) {
    ...
    if($linea != $linea_act){
      ...
      if($linea != ""){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$Fila, 'TOTAL '.$linea);
  ...
  $n = 'Rentabilidad_inv_linea_'.date('Y_m_d_H_i_s');
