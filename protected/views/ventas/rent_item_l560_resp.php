...
//ora
$query_ora = Yii::app()->db->createCommand("SELECT DISTINCT Id_Criterio, Criterio_Descripcion FROM T_CF_CRITERIOS_ITEMS WHERE Id_Plan=950 AND Id_Criterio = '".$ora."'")->queryRow();

$condicion_oracle = $query_ora['Id_Criterio'];
$texto_oracle = $query_ora['Criterio_Descripcion'];
...
$query ="
  SET NOCOUNT ON
  EXEC P_PR_COM_RT_L560_ORACLE_FECH
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(200,5,'RENTABILIDAD POR ITEM LISTA 560',0,0,'L');
    ...
    function Tabla(){
      ...
      $this->Cell(21,5,number_format(($COSTO_VENTA_st),2,".",","),0,0,'R');
  ...
  $pdf->setSql($query);
  $pdf->setOracle($texto_oracle);
  $pdf->AliasNbPages();
  $pdf->AddPage();
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_item_l560_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Rentabilidad_item_l560_'.date('Y_m_d_H_i_s');
