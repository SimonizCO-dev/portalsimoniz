...
$query= "
EXEC P_PR_COM_REDES_RQM_FECH_MARCA
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->Cell(130,5,utf8_decode('PEDIDOS PENDIENTES POR DESPACHO Y REQUISICIONES / MARCA'),0,0,'L');
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        if($marca != $marca_act){
          ...
          if($marca != ""){
            $this->SetFont('Arial','B',5);
            $this->Cell(88,3,'TOTAL '.$marca,0,0,'R');
  ...
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Ped_pend_des_rqm_marca_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $n = 'Ped_pend_des_rqm_marca_'.date('Y_m_d_H_i_s');
