...
$query ="
  SET NOCOUNT ON
  EXEC P_PR_COM_RT_INV_ORACLE_FECH
  @FECHA1 = N'".$FechaM1."',
  @FECHA2 = N'".$FechaM2."',
  @ORACLE1 = N'".$des_ora_ini."',
...
if($opcion == 1){
  ...
  class PDF extends FPDF{
    ...
    function setOracleInicial($des_ora_ini){
      $this->des_ora_ini = $des_ora_ini;
    }

    function setOracleFinal($des_ora_fin){
      $this->des_ora_fin = $des_ora_fin;
    }
    ...
    function Header(){
      ...
      $this->Cell(200,5,'RENTABILIDAD DE INVENTARIO POR ORACLE',0,0,'L');
      ...
      $this->Ln();
      $this->Cell(340,5,utf8_decode('Criterio de búsqueda: Desc. oracle de '.trim($this->des_ora_ini).' a '.trim($this->des_ora_fin)),0,0,'L');
      $this->Ln();
      $this->Ln();
      ...
      $this->Cell(10,2,utf8_decode('ORACLE'),0,0,'L');
    ...
    function Tabla(){
      ...
      foreach ($query1 as $reg1) {
        ...
        if($oracle != $oracle_act){
          ...
          if($oracle != ""){
            ...
            $this->SetFont('Arial','B',6);
            $this->Cell(50,5,'TOTAL '.$oracle,0,0,'R');
      ...
      //se imprime el total de la ultima oracle
      $this->SetFont('Arial','B',6);
      $this->Cell(50,5,'TOTAL '.$oracle,0,0,'R');
      $this->Cell(22,5,number_format(($VLR_VENTA_sp),2,".",","),0,0,'R');
      $this->Cell(19,5,number_format(($VLR_DEVOLUCION_sp),2,".",","),0,0,'R');
  ...
  $pdf->setSql($query);
  $pdf->setOracleInicial($des_ora_ini);
  $pdf->setOracleFinal($des_ora_fin);
  $pdf->AliasNbPages();
  $pdf->AddPage();
  $pdf->Tabla();
  ob_end_clean();
  $pdf->Output('D','Rentabilidad_inv_oracle_'.date('Y_m_d_H_i_s').'.pdf');
}
...
if($opcion == 2){
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'ORACLE / ITEM');
  ...
  foreach ($query1 as $reg1) {
    ...
    if($oracle != $oracle_act){
      ...
      if($oracle != ""){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$Fila, 'TOTAL '.$oracle);
      ...
      $Fila = $Fila + 1;
      $oracle = $reg1['ORACLE'];
      $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$Fila, $oracle);
  ...
  $n = 'Rentabilidad_inv_oracle_'.date('Y_m_d_H_i_s');
