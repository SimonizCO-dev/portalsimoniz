<h4>Verificación entrega física de recibos</h4>
...
<div class="row mb-2">
    <div class="col-sm-12">  
        ...
        <button type="submit" class="btn btn-primary btn-sm" onclick="return valida_opciones(event);" id="btn_submit"><i class="fas fa-check"></i> Verificar recibo(s) seleccionado(s)</button>
    </div>
</div>   
...
function buscarrecibos(){
  ...
  if(filtro == ''){
    ...
    cargarrecibosentfis(filtro);
  }

}

function cargarrecibosentfis(filt){
  ...
  $.ajax({ 
    type: "POST", 
    url: "<?php echo Yii::app()->createUrl('controlrecibos/CargarRecibosEntFis'); ?>",
    data: data,
    dataType: 'json',
