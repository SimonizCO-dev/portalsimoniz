...
class EvaluacionEmpleadoController extends Controller
{
	...
	public function actionCreate($e)
	{
		...
		$tipos = Yii::app()->db->createCommand('SELECT d.Id_Dominio, d.Dominio FROM T_PR_DOMINIO d WHERE Id_Padre = '.Yii::app()->params->tipo_evaluacion.' AND Estado = 1 ORDER BY d.Dominio')->queryAll();
		...
		if(isset($_POST['EvaluacionEmpleado']))
		{
			...
			if($model->save()){
				Yii::app()->user->setFlash('success', "Se registro la evaluación del empleado correctamente.");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['EvaluacionEmpleado']))
		{
			...
			$model->Fecha_Actualizacion = date('Y-m-d H:i:s');
			if($model->save()){
				Yii::app()->user->setFlash('success', "Se actualizo la evaluación del empleado correctamente.");
