...
class CPtjVentasController extends Controller
{
	...
	public function actionOffConfig($id)
	{
		...
		$model->ESTADO = 0;
		if($model->save()){
			Yii::app()->user->setFlash('success', "El porcentaje de recaudo ".$model->ROWID." fue inactivado correctamente.");
			$this->redirect(array('admin'));
		}
	}
	...
	public function actionVerifConfig()
	{
		...
		$q = Yii::app()->db->createCommand("SELECT ROWID FROM T_PR_C_PTJ_VENTAS WHERE TIPO = ".$tipo." AND ESTADO = 1")->queryRow();
