...
/**
 * This is the model class for table "T_PR_I_TERCERO".
 *
 * The followings are the available columns in table 'T_PR_I_TERCERO':
 * @property integer $Id
...
class ITercero extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('Id, Id_Tipo, Nit, Nombre, Telefono, Direccion, Ciudad, Estado, Id_Usuario_Creacion, Id_Usuario_Actualizacion, Fecha_Creacion, Fecha_Actualizacion, orderby', 'safe', 'on'=>'search'),
		);
	}
	...
	public function searchByTercero($filtro) {
        ...
        $resp = Yii::app()->db->createCommand("
		    SELECT TOP 10 Id, CONCAT (Nit, ' - ', Nombre) AS Descr FROM T_PR_I_TERCERO WHERE (Nit LIKE '%".$filtro."%' OR Nombre LIKE '%".$filtro."%') AND Estado = 1 ORDER BY Descr 
		")->queryAll();
        return $resp;
 	...
 	public function searchById($filtro) {
        ...
        $resp = Yii::app()->db->createCommand("
		    SELECT Id, CONCAT (Nit, ' - ', Nombre) AS Descr FROM T_PR_I_TERCERO WHERE Id = '".$filtro."'")->queryAll();
        return $resp;
