...
/**
 * This is the model class for table "TH_DIN_COM_CLIENTE".
 *
 * The followings are the available columns in table 'TH_DIN_COM_CLIENTE':
 * @property integer $Id
 ...
 */
class DinComCliente extends CActiveRecord
{
	/**
	...
	public static function DescPlan($id_plan){
		...
		$q = Yii::app()->db->createCommand("SELECT DISTINCT Plan_Descripcion FROM T_CF_CRITERIOS_CLIENTES WHERE Id_Plan = ".$id_plan)->queryRow();
		return $q['Plan_Descripcion'];
	}

	public static function DescCriterio($id_plan, $id_criterio){

		$q = Yii::app()->db->createCommand("SELECT DISTINCT Criterio_Descripcion FROM T_CF_CRITERIOS_CLIENTES WHERE Id_Plan = ".$id_plan." AND Id_Criterio = ".$id_criterio)->queryRow();
	...
	public function relations()
	{
		...
		return array(
			'idDinCom' => array(self::BELONGS_TO, 'DinCom', 'Id_Din_Com'),
		);
	}
	 ...
	 * @return DinComCliente the static model class
	 */
